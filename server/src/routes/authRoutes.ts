...
const generateTokens = (userId: string) => {
  ...
  const refresh = jsonwebtoken.sign({ id: userId }, process.env.JWT_REFRESH_SECRET as string, { expiresIn: '7d' });
...
router.get(
  ...
  async (req: any, res) => {
    // Generate tokens
    const { accessToken, refreshToken: newRefreshToken } = generateTokens(req.user.id);
    ...
    res.redirect(`http://localhost:5173/oauth-success?accessToken=${accessToken}&refreshToken=${newRefreshToken}`);
  }
);
